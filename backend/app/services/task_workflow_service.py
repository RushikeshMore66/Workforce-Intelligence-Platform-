...
class TaskWorkflowService:
    ...
    All transitions are validated against the canonical workflow graph.
    The transition, activity record, and progress recalculation are written
    in a single database transaction to ensure consistency.
    """
    ...
    def transition(
        ...
        """Attempt a status transition on the given task.

        Row-level lock prevents concurrent transitions from creating
        an inconsistent transition history.
        """
        task = (
            self.db.query(Task)
            .with_for_update()
        ...
        # Workers cannot cancel tasks — only management roles can.
        if (
        ...
        # A reason is mandatory when placing a task on hold.
        ...
        # Progress recalculation is part of the same transaction so the
        # project progress is never out of sync with the task states.
