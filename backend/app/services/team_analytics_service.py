...
class TeamAnalyticsService:
    ...
    def get_team_analytics(self, team_id: str) -> TeamAnalyticsOut:
        workforce = self._get_workforce_metrics(team_id)
        workload = self._get_workload_metrics(team_id)
        activity = self._get_activity_metrics(team_id)
        delivery = self._get_delivery_metrics(team_id)
    ...
    def _get_workforce_metrics(self, team_id: str) -> TeamWorkforceMetrics:
        # Workforce base counts
        ...
        ).filter(Worker.team_id == team_id).first()

        total_workers = workers_query.total or 0
        active_workers = int(workers_query.active or 0)

        # Workers with tasks
        workers_with_tasks = self.db.query(func.count(func.distinct(Task.assignee_id))).join(
            Worker, Task.assignee_id == Worker.id
        ).filter(Worker.team_id == team_id).scalar() or 0

        # Workers with overdue tasks
        workers_with_overdue_tasks = self.db.query(func.count(func.distinct(Task.assignee_id))).join(
            Worker, Task.assignee_id == Worker.id
        ).filter(
        ...
        return TeamWorkforceMetrics(
            total_workers=total_workers,
            active_workers=active_workers,
            workers_with_tasks=workers_with_tasks,
            workers_with_overdue_tasks=workers_with_overdue_tasks,
        )

    def _get_workload_metrics(self, team_id: str) -> TeamWorkloadMetrics:
        status_counts = self.db.query(
            Task.status, func.count(Task.id)
        ).join(
            Worker, Task.assignee_id == Worker.id
        ).filter(Worker.team_id == team_id).group_by(Task.status).all()
        ...
        overdue = self.db.query(func.count(Task.id)).join(
            Worker, Task.assignee_id == Worker.id
        ).filter(
            Worker.team_id == team_id,
            Task.status != TaskStatusEnum.COMPLETED,
    ...
    def _get_activity_metrics(self, team_id: str) -> TeamActivityMetrics:
        now = datetime.utcnow()
        seven_days_ago = now - timedelta(days=7)
        thirty_days_ago = now - timedelta(days=30)

        # Aggregate updates for all workers in the team
    ...
    def _get_delivery_metrics(self, team_id: str) -> TeamDeliveryMetrics:
        # Total tasks and completed tasks for the team
